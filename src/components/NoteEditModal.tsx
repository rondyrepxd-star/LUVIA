...
import { 
  X, ChevronRight, FileText, Heart, Zap, Star, Shield, Anchor, Rocket,
  Book, Code, Globe, Music, GraduationCap, FlaskConical, Calculator, 
  Languages, Palette, Activity, Brain, Compass, Microscope, Hourglass,
  Plus, Check, Library, Save, Notebook, NotebookPen, Moon, Sun, Coffee, PenTool
} from 'lucide-react';
...
const icons = [
  ...
  { id: 'Anchor', icon: <Anchor size={20} /> },
  { id: 'Rocket', icon: <Rocket size={20} /> },
  { id: 'Library', icon: <Library size={20} /> },
  { id: 'Save', icon: <Save size={20} /> },
  { id: 'Notebook', icon: <Notebook size={20} /> },
  { id: 'NotebookPen', icon: <NotebookPen size={20} /> },
  { id: 'Moon', icon: <Moon size={20} /> },
  { id: 'Sun', icon: <Sun size={20} /> },
  { id: 'Coffee', icon: <Coffee size={20} /> },
  { id: 'PenTool', icon: <PenTool size={20} /> },
...
const presetColors = [
  ...
  '#448AFF', // Blue (Zap)
  '#4CAF50', // Green (Star)
  '#FFC107', // Yellow (Shield)
  '#7C4DFF', // Purple (Anchor)
  '#E040FB', // Pink (Rocket)
  '#00BCD4', '#FF9800', '#9E9E9E', '#607D8B', '#000000', '#FFFFFF'
];

const NoteEditModal = ({ isOpen, onClose, note, onUpdate }: NoteEditModalProps) => {
  ...
  useEffect(() => {
    if (note) {
      setTitle(note.title);
      setSelectedColor(note.color || '#9980e5');
      setSelectedIcon(note.icon || 'FileText');
    }
  }, [note, isOpen]);
      ...
      <DialogContent className="max-w-[800px] bg-[#0a0a0a] border-none p-0 overflow-hidden rounded-[2rem] shadow-2xl">
        <DialogHeader className="p-8 pb-4 flex flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-4 text-left">
            ...
            >
              {icons.find(i => i.id === selectedIcon)?.icon || <FileText size={24} />}
            </div>
            <div>
        ...
        <div className="p-8 pt-4 grid grid-cols-[1fr_1.2fr] gap-12 text-left">
          ...
          <div className="space-y-12">
              ...
              <div className="bg-[#111] border border-white/5 rounded-[2rem] p-10 flex items-center gap-6 h-[180px] shadow-inner">
                ...
                >
                  <div className="text-white scale-[1.5]">
                    {icons.find(i => i.id === selectedIcon)?.icon || <FileText size={20} />}
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  <h3 className="text-2xl font-black uppercase italic tracking-tighter text-white truncate max-w-[180px]">
                    {truncatePreview(title || 'NUEVA NOTA...')}
                  </h3>
                  <div className="bg-white/5 px-3 py-1 rounded-full w-fit">
                    <span className="text-[9px] font-black uppercase tracking-widest text-white/40">VECT ACTIVO</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 italic">TÍTULO DE LA NOTA</Label>
          ...
          <div className="space-y-12">
            <div className="space-y-6">
              <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 italic">ICONO IDENTIFICACIÓN</Label>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 italic">COLOR DE NOTA</Label>
                <button 
                  type="button"
        ...
        <DialogFooter className="p-8 border-t border-white/5 flex flex-row items-center justify-end gap-8 bg-black/40">
          ...
          >
            <div className="p-1 bg-white/20 rounded-lg">
              {icons.find(i => i.id === selectedIcon)?.icon || <FileText size={16} />}
            </div>
            <span className="font-black text-sm uppercase italic tracking-tighter">GUARDAR CAMBIOS</span>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
