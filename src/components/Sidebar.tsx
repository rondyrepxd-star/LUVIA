          ...
          <div key={sub.id} className="space-y-1">
            ...
            >
              ...
              <div className="flex items-center gap-0.5 shrink-0 pl-1 border-l border-white/5 ml-1">
                ...
                <button onClick={(e) => { e.stopPropagation(); handleExportFolder(sub); }} className="w-6 h-6 flex items-center justify-center hover:bg-primary/20 text-white/40 hover:text-primary rounded-md transition-colors" title="Download Folder"><Download size={10} /></button>
              ...
              <div key={folder.id} className="space-y-1">
                <div draggable onDragStart={(e) => handleFolderDragStart(e, folder.id)} onDragOver={(e) => handleDragOver(e, folder.id, 'folder')} onDrop={(e) => handleFolderDrop(e, folder.id, 'folder')} onDragLeave={() => { setDragOverId(null); setDropPosition(null); }} className={cn("relative flex items-center gap-1.5 px-3 py-2 rounded-xl cursor-pointer group/folder transition-all", folder.isOpen ? "bg-primary/5" : "hover:bg-[#252525]", dragOverId === folder.id && dropPosition === 'inside' && "bg-primary/20")} onClick={() => handleToggleFolder(folder.id)}>
                   ...
                   <div className="flex items-center gap-0.5 shrink-0 pl-1 border-l border-white/5 ml-1">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleExportFolder(folder); }} className="w-6 h-6 flex items-center justify-center hover:bg-primary/20 text-white/40 hover:text-primary rounded-md transition-colors" title="Download Folder"><Download size={10} /></button>
